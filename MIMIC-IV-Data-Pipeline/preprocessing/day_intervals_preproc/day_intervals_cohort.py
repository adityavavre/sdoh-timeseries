...
def get_visit_pts(mimic4_path:str, group_col:str, visit_col:str, admit_col:str, disch_col:str, adm_visit_col:str, use_mort:bool, use_los:bool, los:int, use_admn:bool, disease_label:str,use_ICU:bool):
    ...
    pts['anchor_year_group_mid'] = (pts['anchor_year_group'].str.slice(start=-4).astype(int) + pts['anchor_year_group'].str.slice(start=0, stop=4).astype(int))//2
    ...
    if use_ICU:
        visit_pts = visit[[group_col, visit_col, adm_visit_col, admit_col, disch_col,'los']].merge(
            pts[[group_col, 'anchor_year', 'anchor_age', 'yob', 'min_valid_year', 'dod','gender', 'anchor_year_group_mid']], how='inner', left_on=group_col, right_on=group_col
        )
    else:
...
#     visit_pts = visit_pts.loc[visit_pts['Age'] >= 18]
    ...
    if use_ICU:
        return visit_pts[[group_col, visit_col, adm_visit_col, admit_col, disch_col,'los', 'min_valid_year', 'dod','Age','gender','ethnicity', 'insurance', 'anchor_year_group_mid']]
    else:
...
def extract_data(use_ICU:str, label:str, time:int, icd_code:str, root_dir, disease_label, cohort_output=None, summary_output=None):
    ...
    cols = [group_col, visit_col, admit_col, disch_col, 'Age','gender','ethnicity','insurance','label', 'anchor_year_group_mid']
