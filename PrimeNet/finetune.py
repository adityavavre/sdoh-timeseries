...
parser.add_argument('--output_dir', type=str, required=True, help='Directory to save the finetuned model to')
...
if __name__ == '__main__':
    # args.path = '/data/av38898/sdoh/data/finetune/'
    ...
    if args.pretrain_model is not None:
        ...
        # model.bert.load_state_dict(torch.load('/data/av38898/sdoh/models/' + args.pretrain_model + '.h5')['model_state_dict'])
        model.bert.load_state_dict(torch.load(args.pretrain_model)['model_state_dict'])
    ...
    for itr in tqdm(range(1, args.niters + 1)):
        ...
        if args.task == 'classification':
            val_loss, val_acc, val_auc, val_true, val_pred = utils.evaluate_classifier(model, val_loader, args=args, dim=dim)
            test_loss, test_acc, test_auc, test_true, test_pred = utils.evaluate_classifier(model, test_loader, args=args, dim=dim)
            val_rec = utils.pos_recall(val_true, val_pred)
            test_rec = utils.pos_recall(test_true, test_pred)
        ...
        if args.task == 'classification':
            ...
            print()
            print('Iter: {}, loss: {:.4f}, acc: {:.4f}, val_loss: {:.4f}, val_acc: {:.4f}, test_acc: {:.4f}, test_auc: {:.4f}, val_rec: {:.4f}, test_rec: {:.4f}'
                .format(itr, train_loss/train_n, train_acc/train_n, val_loss, val_acc, test_acc, test_auc, val_rec, test_rec), end = '\n')
        ...
        if itr % 1 == 0 and args.save:
            torch.save({
                ...
                'loss': -loss,
            }, args.output_dir + 'finetuned_model.h5')
...
if args.task == 'classification':
    ...
    import pickle
    with open('results/val_true.pkl', 'wb') as f:
        pickle.dump(best_val_true, f)
    with open('results/val_pred.pkl', 'wb') as f:
        pickle.dump(best_val_pred, f)
    with open('results/test_true.pkl', 'wb') as f:
        pickle.dump(best_test_true, f)
    with open('results/test_pred.pkl', 'wb') as f:
        pickle.dump(best_test_pred, f)
