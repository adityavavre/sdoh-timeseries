...
def evaluate_classifier(model, test_loader, dec=None, args=None, classifier=None,
    ...
    true = []
    test_loss = 0
    for test_batch, label in tqdm(test_loader):
    ...
    return test_loss/pred.shape[0], acc, auc, true, pred.argmax(1)
...
def get_finetune_data(args):
    ...
    if args.oversample_pos:
        REPEAT = 1
        for _ in range(REPEAT):
            pos_X_train = X_train[y_train==1].clone()
            X_train = torch.cat((X_train, pos_X_train), dim=0)
            y_train = torch.cat((y_train, torch.tensor([1]*pos_X_train.shape[0])), dim=0)
...
def pos_recall(true, pred):
    pred_pos = pred[true==1]
    return pred_pos.sum()/pred_pos.shape[0]
